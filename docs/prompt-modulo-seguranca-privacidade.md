# PROMPT — Módulo de Segurança e Privacidade (App de Saúde & Performance)
...
Você é um **designer de produto sênior + arquiteto de segurança** especializado em apps que lidam com dados sensíveis (saúde, biométricos, nutricionais e de desempenho). Projete o **Módulo de Segurança e Privacidade** de um app mobile-first (iOS/Android) que armazena dados pessoais, de saúde, treinos, medidas corporais e histórico de atividades.
...
- **Login social** (Google One-tap / Sign in with Apple) como alternativa: pedir **apenas** nome, e-mail e foto; suporte a Apple Private Relay para ocultar e-mail.
...
- **Proposta:** distinga "biometria de acesso" (desbloquear o app) de "biometria de aprovação" (confirmar ações críticas — exportar dados de saúde, revogar dispositivo, excluir conta), com fricção proporcional ao risco.
...
- Dispare MFA só em: novo dispositivo, troca de senha/e-mail, exclusão de conta, exportação sensível.
...
- Chaves via Keystore / Keychain. **Proposta:** para relatórios de saúde e exportações, considere criptografia ponta-a-ponta com chave derivada do dispositivo, e comunique isso como badge "E2EE".
...
Eventos: logins novos, tentativas falhas, alteração de senha/e-mail/PIN, exportações, compartilhamentos, restaurações. Notificação imediata em atividade incomum.

### 3.9 Controle de Privacidade

Toggles granulares: perfil público/privado; compartilhamento de estatísticas/conquistas/treinos/progresso; permissões para treinadores/nutricionistas/médicos com revogação a qualquer momento.

### 3.10 Conformidade (LGPD / GDPR)

Consentimento explícito; direito de acesso, correção e exclusão; exportação completa; registro de consentimentos; minimização de coleta. Não pule a consultoria de compliance no design.

### 3.11 IA para Segurança (transparente)

Detecção de padrões incomuns de login, acessos simultâneos distantes, dispositivos desconhecidos; sugestão de troca de senha; ajuste automático do nível de proteção conforme risco.
...
Segurança como **camada transversal**:

- **Dashboard:** ocultar indicadores sensíveis em ambiente público ("modo discreto").
- **Metas:** proteção contra alteração.
- **Calendário:** eventos criptografados.
- **Treinos/Nutrição:** histórico e medidas seguros.
- **Analytics:** relatórios com autenticação adicional.
